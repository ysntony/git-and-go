---
name: git-and-go
description: Use when helping anyone work repo-first: set up or use a personal GitHub repo, onboard existing work into a repo, create or update project folders, turn scattered notes/files/chat context into durable repo artifacts, review diffs, commit, push, and prepare pull request context. This skill does not teach domain strategy; it coaches AI-native, Git-based workflow with the repo as the source of truth.
---
...
- **Personal repo is enough to start**: If no shared repo exists, help the user use their own GitHub repo as the repo-first workspace.
- **Bootstrap the repo when needed**: If the user has no local repo or remote, help create or connect a personal GitHub repo with `gh` when available.
...
- **Ship the loop**: When the user asks to ship, carry the work through `status` -> `diff` -> `add` -> `commit` -> `push` -> pull request context when permissions allow.
...
1. **Orient**
   ...
   - If there is no shared repo, help the user use or create a personal GitHub repo before organizing work.
   - If the user needs a GitHub repo created or connected, read `references/github-repo-bootstrap.md`.
...
5. **Commit and Push**
   ...
   - Push and prepare pull request context when the repo and permissions allow.

## Common Modes

- **Start Work**: Turn an idea, cloud doc, chat summary, or loose notes into a project folder and initial artifact.
- **Bootstrap Repo**: Create or connect a personal GitHub repo, preferably with `gh`, then clone or set `origin`.
- **Onboard Existing Work**: Turn an in-progress project from local files, cloud docs, chat, sheets, or scattered links into a repo project folder, then commit, push, and prepare pull request context when asked.
- **Update Work**: Add progress, decisions, links, data notes, or next steps to an existing project.
- **Ship Work**: Review diff, write commit message, commit, push, and prepare pull request context so the work becomes visible and reviewable.
...
- Read `references/github-repo-bootstrap.md` when the user needs a GitHub repo created, cloned, initialized, or connected as `origin`.
- Read `references/existing-work-onboarding.md` when converting already-running work into repo artifacts.
- Read `references/git-workflow.md` before committing, pushing, or preparing a pull request.
...
- commit/push/pull request status, if requested
